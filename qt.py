"""Public widget helper: two linked province/municipality combo boxes.

``CubaProvinces_BoxGroup`` fills a province ``QComboBox`` and, when a
municipality box is also provided, keeps it in sync with the selected
province. It can either drive combo boxes you already created or build them
...
class CubaProvinces_BoxGroup():
    """Link a province combo box to a municipality combo box.

    Args:
        provinceBox: Existing province ``QComboBox`` (caller-managed mode).
        municipalityBox: Existing municipality ``QComboBox`` (caller-managed mode).
        parent: Parent widget. If given, both combo boxes are created and
    ...
    def __buildComboBox(self, parent):
        """Create and position both combo boxes under ``parent``."""
        # Possibly wrap these in a layout or a group box later.
    ...
    def __linkTogether(self, province_opt):
        """Refill the municipality box for the selected province index."""
